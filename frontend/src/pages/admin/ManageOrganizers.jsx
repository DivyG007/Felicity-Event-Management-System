...
export default function ManageOrganizers() {
    ...
    const handleRemove = async (id) => {
        if (!confirm('Are you sure? This will disable the organizer.')) return;
        try { await removeOrganizer(id); fetchOrgs(); }
            ...
            {loading ? <div className="empty-state"><div className="spinner"></div></div> : (
                <div className="card">
                    <table className="data-table">
                        <thead><tr><th>Name</th><th>Category</th><th>Email</th><th>Status</th><th>Actions</th></tr></thead>
                        <tbody>
                            {organizers.map(o => (
                                <tr key={o._id}>
                                    ...
                                    <td>
                                        <button className="btn btn-danger" style={{ fontSize: '0.75rem', padding: '0.3rem 0.6rem' }} onClick={() => handleRemove(o._id)}>
                                            {o.active ? 'Disable' : 'Remove'}
                                        </button>
                                    </td>
                                </tr>
                            ))}
